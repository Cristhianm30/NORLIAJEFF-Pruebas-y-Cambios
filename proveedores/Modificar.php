...
if (!empty($_POST['BtnRegistrar'])) {
    ...
    if (!empty($_POST['id'])  && !empty($_POST['nombre']) && !empty($_POST['correo']) && !empty($_POST['telefono']) ) {
        ...
        $sql = $conexion->prepare("UPDATE proveedor SET nombre = ?, correo = ?, telefono = ? WHERE id = ?");
        $sql->bind_param("sssi", $nombre, $correo, $telefono, $id);
        $sql->execute();
...
if (!empty($_GET['id'])) {
    $id_producto = $_GET['id'];
    $sql_producto = "SELECT * FROM proveedor WHERE id = $id_producto";
    $result_producto = $conexion->query($sql_producto);
    if ($result_producto->num_rows > 0) {
        // Obtener los datos del producto
        $row_proveedor = $result_producto->fetch_assoc();
    } else {
...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
    ...
    <main>
        <form class="col-4 p-4 m-auto" method="POST">
            <h2 class="text-center" id="titulo">Modificar Proveedor</h2>
            <!-- Campo oculto para enviar el ID del producto -->
            <input type="hidden" name="id" value="<?php echo $row_proveedor['id']; ?>">
            <!-- Campo para el nombre del producto -->
            <div class="mb-3">
                <label for="exampleInputEmail1" class="form-label">Id del Proveedor</label>
                <input type="text" class="form-control" name="id" id="id" required autofocus value="<?php echo $row_proveedor['id']; ?>">
            </div>

            <div class="mb-3">
                <label for="exampleInputEmail1" class="form-label">Nombre del Proveedor</label>
                <input type="text" class="form-control" name="nombre" id="nombre1" required autofocus value="<?php echo $row_proveedor['nombre']; ?>">
            </div>
            <!-- Campo para el precio del producto -->
            <div class="mb-3">
                <label for="exampleInputEmail1" class="form-label">Correo</label>
                <input type="text" class="form-control" name="correo" id="correo" required value="<?php echo $row_proveedor['correo']; ?>">
            </div>
            <!-- Campo para el proveedor del producto -->
            <div class="mb-3">
                <label for="exampleInputEmail1" class="form-label">Telefono</label>
                <input type="text" class="form-control" name="telefono" id="telefono" required value="<?php echo $row_proveedor['telefono']; ?>">
            </div>
