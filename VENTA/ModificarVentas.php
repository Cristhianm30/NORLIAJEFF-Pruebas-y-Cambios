...
if (!empty($_POST['BtnRegistrar'])) {
    ...
    if (!empty($_POST['id']) && !empty($_POST['codigo']) && !empty($_POST['producto']) && !empty($_POST['cantidad']) && !empty($_POST['monto']) && !empty($_POST['fecha']) ) {
        ...
        $sql = $conexion->prepare("UPDATE ventas SET codigo = ?, producto = ?, cantidad = ?, monto = ?, fecha = ? WHERE id = ?");
        $sql->bind_param("isissi", $codigo, $producto, $cantidad, $monto, $fecha,  $id);
        $sql->execute();
...
if (!empty($_GET['id'])) {
    $id_compra = $_GET['id'];
    $sql_compra = "SELECT * FROM ventas WHERE id = $id_compra";
...
$conexion->close();
?>
        <h2 class="text-center" id="titulo">Modificar Venta</h2>
